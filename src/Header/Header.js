				...
				<Modal.Body>
					<form onSubmit={EventSubmit}>
						<div className="input-group mb-0 w-75 mx-auto">
							<input type="titleText" placeholder="Title" name="titleText" id='title' />
						</div>

						<div className="input-group mb-0 w-75 mx-auto">
							<input type="urlText" placeholder="Url" name="urlText" id='url' />
						</div>
						<div className="input-group">
							<label for="className">Catagory </label>
							<select name="className" type="className" id='select'>
								{
									dropdownOptions.map(item => <option value={item} key={item}>Catagory {item}</option>)
								}
							</select>
							<button onClick={() => addOption()}>Plus</button>
						</div>
						<input className='form-submit button-87 w-75 mx-auto' type="submit" required value="Save" />
					</form>
				</Modal.Body>
				<Modal.Footer>
				</Modal.Footer>
